...
# Remove phone 17
Training <- Training[!(Training$PHONEID==17 & Training$BUILDINGID == 2),]
...
# Extract no wapps features
Testing_wapps <- Testing[,grep("WAP", names(Testing), value = T)]

# Replace x <= 0 & x > -30 (phone 19 / user 6) | x == 100 by -105
x <- as.data.frame(apply(Testing[,grep("WAP", names(Testing), value = TRUE)], 2, function(x){ifelse(x <= 0 & x > -30 | x == 100 | x < -96, -105, x)})) #  | x < -96
Testing <- cbind(x, Testing[,grep("WAP", names(Testing), value = TRUE, invert = TRUE)])


# New feature for best value
Testing$MAX_W <- colnames(Testing)[apply(Testing[,grep("WAP", colnames(Testing))],1,which.max)]
...
# Confusion Matrix
confusionMatrix(table(Testing$BUILDINGID, Prediction_B$Predictions$RF))
...
floor.prediction <- function(training, testing) {
  ...
  # bestMtry <- tuneRF(training[,grep("WAP", colnames(training), value = TRUE)], training$FLOOR, ntreeTry=100, stepFactor=2, improve=0.05, trace=TRUE, plot=T)
...
# saving the results of your models into data frames
Prediction_F_B0 <-as.data.frame(Prediction_F$`B 0`$Metrics)
Prediction_F_B1 <-as.data.frame(Prediction_F$`B 1`$Metrics)
Prediction_F_B2 <-as.data.frame(Prediction_F$`B 2`$Metrics)

# saving the best model (rf)
bestMtry <- tuneRF(Training[,grep("WAP", colnames(Training), value = TRUE)], Training$FLOOR, ntreeTry=100, stepFactor=2, improve=0.05, trace=TRUE, plot=T)
Training$FLOOR <- as.factor(Training$FLOOR)
is.factor(Training$FLOOR)
floor_rf <- randomForest(y = Training$FLOOR, x = Training[,grep("WAP", names(Training), value = T)], importance=T, method="rf", mtry = bestMtry)
#saveRDS(floor_rf, "../models/floor_rf.rds")
floor_rf <- readRDS("../models/floor_rf.rds")


# Confusion Matrix of the best model (error!)
confusionMatrix(table(Testing[(Testing$FLOOR & Testing$BUILDINGID == 0),], Prediction_F$`B 0`$Predictions$RF))

length(Testing[(Testing$FLOOR & Testing$BUILDINGID == 0),])
length(Prediction_F$`B 0`$Predictions$RF)


# ADD FLOOR PREDICTION
Data_clean$`B 0`$test$FLOOR_PRED <- Prediction_F$`B 0`$Predictions$RF # KAPPA RF 0.963
Data_clean$`B 1`$test$FLOOR_PRED <- Prediction_F$`B 1`$Predictions$SVM # KAPPA SVM 0.719
Data_clean$`B 2`$test$FLOOR_PRED <- Prediction_F$`B 2`$Predictions$KNN # KAPPA KNN 0.944
...
for (i in Names) {
  for (j in Go) {
    Data_clean[[i]][[j]] <- Data_clean[[i]][[j]][!Data_clean[[i]][[j]]$PHONEID == 17,]
  }
}
...
# lat_knn <- knnreg(Training$LATITUDE ~ ., data = Training)
# saveRDS(lat_knn, "../models/lat_knn.rds")
lat_knn <- readRDS("../models/lat_knn.rds")
lat_knn <- knnreg(Training$LATITUDE ~ ., data = Training)
lat_pred_knn <- predict(lat_knn, Testing)
lat_metrics_knn <- postResample(Testing$LATITUDE, lat_pred_knn)
...
Data_clean$`B 0`$test$LATT_PRED <- Prediction_Latt$`B 0`$Predictions$KNN # KNN 3.31
Data_clean$`B 1`$test$LATT_PRED <- Prediction_Latt$`B 1`$Predictions$KNN # KNN 7.21
Data_clean$`B 2`$test$LATT_PRED <- Prediction_Latt$`B 2`$Predictions$KNN # KNN 5.98
...
# long_knn <- knnreg(Training$LONGITUDE ~ ., data = Training)
# saveRDS(long_knn, "../models/long_knn.rds")
long_knn <- readRDS("../models/long_knn.rds")


# ADD LONGITUDE PREDICTION
Data_clean$`B 0`$test$LONG_PRED <- Prediction_Long$`B 0`$Predictions$KNN # KNN 3.74
Data_clean$`B 1`$test$LONG_PRED <- Prediction_Long$`B 1`$Predictions$KNN # KNN 6.44
Data_clean$`B 2`$test$LONG_PRED <- Prediction_Long$`B 2`$Predictions$KNN # KNN 7.09
...
# Save in csv
write.csv(FINAL, file = "../datasets/Prediction.csv")
